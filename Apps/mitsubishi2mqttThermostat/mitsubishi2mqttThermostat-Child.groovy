...
def installed() {
	...
	try {
		...
		//thermostat = addChildDevice("dtherron", "Advanced Mitsubishi2Mqtt Thermostat Device", state.deviceID, null, [label: label, name: label, completedSetup: true]) //** Deprecated hubIDl no longer passed since 2.1.9
		thermostat = addChildDevice("dtherron", "Advanced Mitsubishi2Mqtt Thermostat Device", state.deviceID, [label: label, name: label, completedSetup: true]) //** This will only work with ver 2.1.9 and up, let's wait a bit
	} catch(e) {
